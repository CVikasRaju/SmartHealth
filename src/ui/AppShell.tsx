...
function ActivePatientSelector() {
  ...
  if (role === "patient") {
    ...
    return (
      <div className="hidden items-center gap-2 border border-rule bg-paper px-2.5 py-1.5 md:flex rounded-sm">
...
function RiskSummary() {
  ...
  if (portfolio.critical + portfolio.high === 0) {
    return (
      <span className="hidden items-center gap-2 border border-risk-normal/40 bg-risk-normal/[0.06] px-2.5 py-1.5 text-xs text-risk-normal lg:flex rounded-sm">
  ...
  return (
    <span className="hidden items-center gap-2 border border-risk-critical/40 bg-risk-critical/[0.06] px-2.5 py-1.5 text-xs text-risk-critical lg:flex rounded-sm">
...
function SyncIndicator() {
  ...
  if (sync.error) {
    return (
      <button
        ...
        title={sync.error}
        className="inline-flex items-center gap-1.5 border border-risk-critical/40 bg-risk-critical/[0.06] px-2.5 py-1.5 text-xs text-risk-critical rounded-sm transition hover:bg-risk-critical/[0.12]"
      >
  ...
  if (sync.pending > 0) {
    return (
      <span className="inline-flex items-center gap-1.5 border border-rule bg-paper px-2.5 py-1.5 text-xs text-ink-500 rounded-sm">
        <span className="h-3 w-3 animate-spin border border-ink-300 border-t-accent rounded-full" />
        Saving
      </span>
    );
  }
...
export default function AppShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-canvas text-ink-900">
      ...
      {/* ---------------------------------------------------------- */}
      {/* Masthead: Double-ruled institutional letterhead            */}
      {/* ---------------------------------------------------------- */}
      <header className="no-print sticky top-0 z-30 bg-paper sm-masthead-rule">
        <div className="mx-auto flex max-w-[110rem] flex-wrap items-center gap-4 px-4 py-2.5 lg:px-6">
          <div className="flex items-center gap-3">
            <span
              className="grid h-10 w-10 shrink-0 place-items-center rounded-sm bg-accent text-white shadow-xs"
            ...
            <div className="leading-tight">
              ...
              <p className="text-[10px] font-medium uppercase tracking-[0.14em] text-ink-400">{BRANDING.tagline}</p>
            </div>
          </div>
          ...
          <div className="ml-auto flex flex-wrap items-center gap-2">
            ...
            <RiskSummary />
            <span className="hidden items-center gap-1.5 border border-rule bg-paper px-2.5 py-1.5 text-xs text-ink-500 rounded-sm xl:flex">
        ...
        <div className="border-t border-rule bg-accent-soft/40">
      ...
      <div className="mx-auto flex w-full max-w-[110rem] flex-1 flex-col gap-5 px-4 py-4 lg:flex-row lg:px-6">
            ...
            {nav.map((item) => {
              ...
              return (
                <button
                  ...
                  className={cx(
                    "flex w-full min-w-max items-center gap-2.5 rounded border px-3 py-2 text-left transition lg:min-w-0",
                    isActive
                      ? "border-accent/40 bg-paper text-accent font-semibold shadow-panel"
                      : "border-transparent text-ink-600 hover:border-rule hover:bg-paper/70 hover:text-ink-900",
                  )}
                >
                  ...
                  <span className="min-w-0">
                    <span className={cx("block truncate text-xs", isActive ? "font-semibold text-ink-900" : "font-medium text-ink-700")}>
                      {item.label}
                    </span>
          ...
          <div className="no-print mt-4 hidden space-y-3 lg:block">
            ...
            <div className="rounded border border-rule bg-paper p-3 shadow-panel">
              ...
              <div className="mt-2 flex items-center gap-2.5">
                <span
                  className="grid h-8 w-8 shrink-0 place-items-center rounded-sm text-[11px] font-bold text-white"
        ...
        <main className="min-w-0 flex-1">
          <div className="no-print mb-4 border-b border-rule pb-3">
            ...
            <h1 className="mt-1 font-serif text-[26px] font-semibold leading-tight text-ink-900">{active.label}</h1>
